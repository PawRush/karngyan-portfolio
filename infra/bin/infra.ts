...
const repositoryName = app.node.tryGetContext("repositoryName") || "PawRush/karngyan-portfolio";
const branchName = app.node.tryGetContext("branchName") || "deploy-to-aws-20260501_121659-kamielw";
...
if (!codeConnectionArn) {
  ...
  new FrontendStack(app, `KarnGyanFrontend-${environment}`, {
    env: { account, region },
    environment,
    buildOutputPath,
...
if (codeConnectionArn) {
  new PipelineStack(app, "KarngyanPipelineStack", {
    env: { account, region },
    description: "CI/CD Pipeline for Karngyan Portfolio",
    codeConnectionArn,
    repositoryName,
    branchName,
...
cdk.Tags.of(app).add("Project", "KarnGyan");
