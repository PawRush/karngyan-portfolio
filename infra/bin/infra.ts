...
if (!codeConnectionArn) {
  ...
  new FrontendStack(app, `KarnPortfolioFrontend-${environment}`, {
    env: { account, region },
    environment,
    buildOutputPath,
...
if (codeConnectionArn) {
  new PipelineStack(app, "KarnPortfolioPipelineStack", {
    env: { account, region },
    description: "CI/CD Pipeline for KarnPortfolio",
    codeConnectionArn,
    repositoryName,
    branchName,
...
cdk.Tags.of(app).add("Project", "KarnPortfolio");
